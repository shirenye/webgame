package com.webgame.auth.translator;
...
@Slf4j
@Component
public class WebGameWebResponseExceptionTranslator implements WebResponseExceptionTranslator {

    @Override
    public ResponseEntity<?> translate(Exception e) {
        ResponseEntity.BodyBuilder status = ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR);
        WebGameResponse response = new WebGameResponse();
        String message = "认证失败";
        log.error(message, e);
