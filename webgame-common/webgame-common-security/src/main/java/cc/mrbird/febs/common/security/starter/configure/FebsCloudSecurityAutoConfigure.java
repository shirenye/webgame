...
public class FebsCloudSecurityAutoConfigure extends GlobalMethodSecurityConfiguration {
    ...
    public RequestInterceptor oauth2FeignRequestInterceptor() {
        return requestTemplate -> {
            ...
            requestTemplate.header(WebGameConstant.GATEWAY_TOKEN_HEADER, gatewayToken);
            String authorizationToken = WebGameUtil.getCurrentTokenValue();
            if (StringUtils.isNotBlank(authorizationToken)) {
                requestTemplate.header(HttpHeaders.AUTHORIZATION, WebGameConstant.OAUTH2_TOKEN_TYPE + authorizationToken);
            }
        };
    }
